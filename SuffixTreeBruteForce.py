...
class SuffixTreeBruteForce:
            ...
            while(j < len(s)):
                if s[j] in curr.nexts:
                    ...
                    if label_end:
                        ...
                        # we create a node here with the left half of the label (the part that matched) and replace child node's position with it
    ...
    def findSubstring(self, s):
        ...
        and returns true if the string is found, time complexity for this function is O(len(s))
        """
        curr = self.root
        i = 0
        while(i < len(s)):
